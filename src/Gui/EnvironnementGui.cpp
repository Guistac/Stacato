...
void environnementGui() {
	...
	if (machineList.beginList("##MachineList", glm::vec2(ImGui::GetTextLineHeight() * 15.0, ImGui::GetContentRegionAvail().y))) {
		...
		for (int i = 0; i < machines.size(); i++) {
			...
			if (machineList.beginItem(glm::vec2(ImGui::GetTextLineHeight() * 14.0, 100))) {
	...
	if (etherCatDeviceList.beginList("##EtherCatDeviceList", glm::vec2(ImGui::GetTextLineHeight() * 15.0, ImGui::GetContentRegionAvail().y))) {
