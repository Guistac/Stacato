...
void EL7221_9014::initialize() {
	rxPdoAssignement.addNewModule(0x1600);
	rxPdoAssignement.addEntry(0x7010, 0x1, 16, "DRVControlWord", &controlWord);							//DS402 control word
	rxPdoAssignement.addEntry(0x7010, 0x3, 8, "ModeOfOperationSelection", &modeOfOperationSelection);	//Operating Mode Selection
	rxPdoAssignement.addEntry(0x7010, 0x5, 32, "TargetPosition", &targetPosition);						//Target Position
	rxPdoAssignement.addEntry(0x7010, 0x6, 32, "TargetVelocity", &targetVelocity);						//Target Velocity
	
	txPdoAssignement.addNewModule(0x1A00);
	txPdoAssignement.addEntry(0x6010, 0x1, 16, "StatusWord", &statusWord);									//DS402 status word
	txPdoAssignement.addEntry(0x6010, 0x3, 8, "ModeOfOperationDisplay", &modeOfOperationDisplay);			//Operating Mode Display
	txPdoAssignement.addEntry(0x6000, 0x11, 32, "ActualPosition", &fbPosition);								//Fb Position
	txPdoAssignement.addEntry(0x6010, 0x6, 32, "FollowingErrorActualValue", &followingErrorActualValue);	//following error actual value
	txPdoAssignement.addEntry(0x6010, 0x7, 32, "VelocityActualValue", &velocityActualValue);				//Velocity Actual Value
	txPdoAssignement.addEntry(0x6010, 0x8, 16, "TorqueActualValue", &torqueActualValue);					//Torque Actual Value
}
