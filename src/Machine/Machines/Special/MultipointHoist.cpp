...
bool MultipointHoist::AxisMapping::save(tinyxml2::XMLElement* parent){
	parent->SetAttribute("AxisPinSaveName", axisPin->saveString);
	parent->SetAttribute("InvertDirection", b_invertDirection);
	parent->SetAttribute("PositionOffset", positionOffset);
	return true;
}

bool MultipointHoist::AxisMapping::load(tinyxml2::XMLElement* parent){
	...
	parent->QueryAttribute("PositionOffset", &positionOffset);
	return true;
}

bool MultipointHoist::AxisMapping::isAxisConnected(){
	return axisPin->isConnected();
}

std::shared_ptr<AxisInterface> MultipointHoist::AxisMapping::getAxis(){
	if(!axisPin->isConnected()) return nullptr;
	return axisPin->getConnectedPin()->getSharedPointer<AxisInterface>();
}

void MultipointHoist::AxisMapping::enableAxis(){
	if(axisPin->isConnected()) getAxis()->enable();
}
void MultipointHoist::AxisMapping::disableAxis(){
	if(axisPin->isConnected()) getAxis()->disable();
}

void MultipointHoist::AxisMapping::updateAnimatableParameters(){
	if(!isAxisConnected()) {
	...
	animatablePosition->lowerPositionLimit = axisPositionToMachinePosition(axis->getLowerPositionLimit());
	animatablePosition->upperPositionLimit = axisPositionToMachinePosition(axis->getUpperPositionLimit());
	if(b_invertDirection){
		std::swap(animatablePosition->lowerPositionLimit, animatablePosition->upperPositionLimit);
	}
...
void MultipointHoist::AxisMapping::updateRealAnimatableValues(){
...
void MultipointHoist::AxisMapping::updateAxisCommand(double profileTime_seconds, double profileDeltaTime_seconds){
